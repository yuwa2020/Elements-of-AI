...
def validate_route(answer, args):
    ...
    hours, delivery_accs, route_taken = answer['total-hours'], answer['total-expected-accidents'], answer['route-taken']
    ...
    assert isinstance(delivery_accs, float), f'{str(delivery_accs)} is not an float: Any probability must be float'
    assert delivery_accs >= 0, f'{str(delivery_accs)} < 0: total-expected-accidents must be positive'
    ...
    return segments, miles, hours, delivery_accs
